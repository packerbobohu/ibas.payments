...
import { DataConverter4py } from "../../borep/DataConverters";
...
export class PaymentListApp extends ibas.BOListApplication<IPaymentListView, bo.Payment> {
    ...
    /** 应用名称 */
    static APPLICATION_NAME: string = "payments_app_payment_list";
    /** 业务对象编码 */
    ...
    /** 删除数据，参数：目标数据集合 */
    protected deleteData(data: bo.Payment): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)) {
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_please_chooose_data",
        ...
        if (data instanceof Array ) {
            for (let item of data) {
                if (ibas.objects.instanceOf(item, bo.Payment)) {
                    item.delete();
                    beDeleteds.add(item);
                }
            }
    ...
    protected getServiceProxies(): ibas.IServiceProxy<ibas.IServiceContract>[] {
        return [
            new ibas.BOListServiceProxy({
                ...
                converter: new DataConverter4py()
            })
        ];
    }
}
